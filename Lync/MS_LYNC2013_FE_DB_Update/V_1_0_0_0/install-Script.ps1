<#
.SYNOPSIS
The script does an update on the Lync Frontend Server database.
...
## 22.09.2016			1.0.0.0		S. Schmalz (IF)			Paket fertiggestellt
## --------------------------------------------------------------------------------------
...
Initialize-BLFunctions -AppName "MS_LYNC2013_FE_DB_Update" -AppSource $Local:ScriptPath -Force:$Force -Uninstall:$Uninstall
...
Function Invoke-ISInstallation() {
	$ExitCode = 1
	$LogType = "Information"
	$LogFile = "C:\RIS\Log\Update_FE_Database.html"
...
#	$Server = hostname
#	$RegKey = "HKLM:\Software\Atos\MS_LYNC2013_FE_DB_Update"
	...
	if ($ExitCode -eq 0) {
		try {
			...
			$UpdateDB = Install-CsDatabase -Update -ConfiguredDatabases -SqlServerfqdn $SqlServerfqdn -Report $LogFile -ErrorAction Stop
			$ExitCode = 0
		} catch {
			$LogType = "Error"
			$ErrorMessage = $_.Exception.Message
			"This error was thrown while updating the 'configured databases' of Lync Frontend Servers. This Error was thrown: `r`n $ErrorMessage"  | Write-BLLog -LogType $LogType
