<#
.SYNOPSIS
(Un-)Installation that are required WindowsFeatures for Lync 2013 Office Web App Server


.DESCRIPTION
Autor: stefan.schmalz@interface-ag.de

The script starts the (un-)installation that are required WindowsFeatures for Lync 2013 Office Web App Server:
...
.OUTPUTS
The script created a logfile: "C:\RIS\Log\MS_LYNC2013_WA_Prereq_WindowsFeatures.log"
...
## 28.07.2016	V0.0.0.2 	S. Schmalz (IF)		Skript fertig
##
...
$InstLogName = "MS_LYNC2013_WA_Prereq_WindowsFeatures_Inst.log"
$DeInstLogName = "MS_LYNC2013_WA_Prereq_WindowsFeatures_Deinst.log"
...
Initialize-BLFunctions -AppName "MS_LYNC2013_WA_Prereq_WindowsFeatures" -AppSource $Local:ScriptPath -Force:$Force -Uninstall:$Uninstall
...
Function Invoke-ISInstallation($LogPathInst,$RebootPending) {
     if (!$RebootPending) {
		...
		#1st Install AS-NET-FRAMEWORK
		"Installing AS-NET-FRAMEWORK first. Invoking: Add-WindowsFeature -Name AS-NET-FRAMEWORK" | Write-BLLog -LogType $LogType
		$Success = Add-WindowsFeature -Name AS-NET-FRAMEWORK
		If ($Success.Success -eq "True") {
			"AS-NET-FRAMEWORK was installed successful, install the remaining windows features." | Write-BLLog -LogType $LogType
			$Success  = Add-WindowsFeature Web-Server, Web-Scripting-Tools, Web-Windows-Auth, Web-Includes, Web-Asp-Net, Web-Log-Libraries, Web-Http-Tracing, Web-Stat-Compression, Web-Default-Doc, Web-ISAPI-Ext, Web-ISAPI-Filter, Web-Http-Errors, Web-Http-Logging, Web-Net-Ext, Web-Client-Auth, Web-Filtering, Web-Mgmt-Console, Web-Dyn-Compression, Ink-Handwriting, IH-Ink-Support, NET-Win-CFAC -LogPath $LogPathInst
			...
			$LogType = "Error"
			"AS-NET-FRAMEWORK was not installed successful, please take a look at the logfile." | Write-BLLog -LogType $LogType
			Return 1
		}
    } else {
        $LogType = "Error"
...
Function Invoke-ISUninstallation($LogPathDeInst,$RebootPending) {
    if (!$RebootPending) {
        ...
        $LogType = "Information"
		    ...
		    $Success = Remove-WindowsFeature Web-Server, Web-Scripting-Tools, Web-Windows-Auth, Web-Includes, Web-Asp-Net, Web-Log-Libraries, Web-Http-Tracing, Web-Stat-Compression, Web-Default-Doc, Web-ISAPI-Ext, Web-ISAPI-Filter, Web-Http-Errors, Web-Http-Logging, Web-Net-Ext, Web-Client-Auth, Web-Filtering, Web-Mgmt-Console, Web-Dyn-Compression, Ink-Handwriting, IH-Ink-Support, NET-Win-CFAC, AS-NET-FRAMEWORK -LogPath $LogPathDeInst
		    #endregion
		    $ExitCode = $Success.Exitcode
		    If ($Success.Success -eq "True") {
			    $LogType = "Information"
