...
.DESCRIPTION
This script sets two DNS Records for each persistent chat server.
One for the server itself and one for the persistent chat pool and the IP of the server.
...
## 27.09.2016			1.0.0.0 		Stefan Schmalz (IF)			Skript fertig zum Testen
## --------------------------------------------------------------------------------------
...
Initialize-BLFunctions -AppName "MS_LYNC2013_PC_CFG_DNSRecords" -AppSource $Local:ScriptPath -Force:$Force -Uninstall:$Uninstall
...
Function Invoke-ISInstallation() {
	...
	$SetRecords4 = $PCPoolName, $PCServerName
