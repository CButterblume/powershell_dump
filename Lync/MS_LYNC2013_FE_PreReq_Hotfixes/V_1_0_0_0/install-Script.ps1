...
## Date			Version		By					Change Description
## -------------------------------------------------------------------------
## 28.07.2016	V0.0.0.1	S. Schmalz (IF)		Datei angelegt, Skriptkopf und CBH eingefuegt.
## 30.08.2016	V1.0.0.0 	S. Schmalz (IF)		Skript fertig
...
Initialize-BLFunctions -AppName "MS_LYNC2013_FE_Prereq_Hotfixes" -AppSource $Local:ScriptPath -Force:$Force -Uninstall:$Uninstall

"Starting '$AppName' at $((Get-Date).ToLongTimeString())" | Write-BLLog -LogType Information


#region installation.
Function Invoke-ISInstallation ($Ly2013_WinUpdates,$Ly2013_PreReqPath,$Logpath) {
...
## Use only absolute filepaths to access files in package; the variable $AppSource points to the folder of this installation skript.
	...
	if (($ExitCode -eq 0) -OR ($ExitCode -eq 3010)) {
		...
		if ($InstalledKB) {
			...
			"Update $MSUFile is not installed. Uninstallation not necessary." | Write-BLLog
			$ExitCode = 0
		}
	}
	Return $ExitCode
...
Function Invoke-ISUninstallation ($Ly2013_WinUpdates,$Ly2013_PreReqPath,$Logpath) {
	#Setting Variables for Logging
	$Logpath = "C:\RIS\Log\"
	$Ly2013_PreReqPath = $AppSource + "\Source"
...
## ====================================================================================================
	...
	If (-Not $Uninstall) {
		$ExitCode = Invoke-ISInstallation -Ly2013_WinUpdates $Ly2013_WinUpdates -Ly2013_PreReqPath $Ly2013_PreReqPath -LogPath $Logpath
	} Else {
		$ExitCode = Invoke-ISUninstallation  -Ly2013_WinUpdates $Ly2013_WinUpdates -Ly2013_PreReqPath $Ly2013_PreReqPath -LogPath $Logpath
	}
#endregion MAIN
